...
def append_boot_strap(src):
    with open(src, 'w+') as file:
        set_param(file, 'SP', 261)
...
def extracte_code_dir(dir, file_name):
    vm_file_src = dir + file_name + '.vm'
    asm_file_src = dir + file_name + '.asm'
    append_boot_strap(asm_file_src)
    print('target_file_src', vm_file_src)
    with open(vm_file_src, 'a+') as file_target:
...
extracte_code_dir('D:/program/nand2tetris/08/FunctionCalls/FibonacciElement/', 'FibonacciElement')
# extracte_code_dir('D:/program/nand2tetris/08/FunctionCalls/NestedCall/', 'NestedCall')